 ...
 *   model a double-opened tab) -> first answer (submitted TWICE concurrently
 *   with the same submissionId).
 *
 * Two shapes:
 *  - PER_LEARNER_POOL: every learner gets its own `Pool({max: 1})` —
 *    approximates Vercel serverless where each invocation holds its own
 *    one-connection pool (the app's `max: 1`, `pg-pool.ts`).
 *  - SHARED_POOL_1: ONE `Pool({max: 1})` for all learners — the worst-case
 *    "one warm instance serves the whole class" queueing shape.
 *
...
import { buildReport } from "../../../scripts/burst/burst-stats.mjs";
...
interface StepResult {
  ...
  duplicateOutcomes?: { today?: string; answer?: string };
}
...
async function learnerFlow(args: {
  ...
  let step = "join";
  const duplicateOutcomes: StepResult["duplicateOutcomes"] = {};
  await args.start;
  ...
  try {
    ...
    // 2. first Today: TWO concurrent opens (double tab) -> one plan.
    step = "today";
    const now = new Date();
    const openToday = async () => {
      ...
      return getOrCreateDailyPlanForToday(
        { userId, now },
        createProductionDailyPlanGenerationSettings(),
        createProductionDailyPlanPorts(pool, provider),
      );
    };
    t = performance.now();
    const [first, second] = await Promise.all([openToday(), openToday()]);
    ...
    // 3. first answer: submitted TWICE concurrently with the same submissionId.
    step = "answer";
    ...
    const submit = () =>
      submitDailyPlanItemAnswer(
        ...
        },
        {
          ...
          uow: new PostgresUnitOfWork(new PgConnectionProvider(pool)),
        },
      );
    t = performance.now();
    const [a, b] = await Promise.all([submit(), submit()]);
    steps.answer = performance.now() - t;
    const outcomes = [a, b].map((r) => r.kind);
    duplicateOutcomes.answer = outcomes.join("+");
    if (!outcomes.includes("ACCEPTED")) throw new Error(`no ACCEPTED answer outcome: ${outcomes.join(",")}`);
    if (outcomes.some((k) => k !== "ACCEPTED")) {
      throw new Error(`duplicate same-submission answer produced ${outcomes.join(",")}`);
    }
    return { ok: true, steps, duplicateOutcomes };
  } catch (error) {
    const e = error as { message?: string; code?: string };
    return { ok: false, steps, failure: { step, message: e.message, code: e.code }, duplicateOutcomes };
  }
}
...
async function runScenario(scenario: "PER_LEARNER_POOL" | "SHARED_POOL_1") {
  ...
  const flows = learnerIds.map((userId) => {
    const pool = sharedPool ?? new pg.Pool({ connectionString: url, max: 1 });
    if (!sharedPool) pools.push(pool);
    return learnerFlow({ pool, userId, courseId, start });
  });
  ...
  const report = buildReport({
    ...
    extra: {
      ...
      poolShape: sharedPool ? "1 shared Pool(max:1)" : `${learnerIds.length} x Pool(max:1)`,
      invariants,
    },
  });
