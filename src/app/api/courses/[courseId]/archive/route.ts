/**
 * POST /api/courses/:courseId/archive — Run 005 S2 archive transition.
 * Deliberately thin, mirrors `courses/[courseId]/publish/route.ts` exactly.
 *
 * Explicit Node runtime: `pg` has no Edge-runtime build.
 *
 * ## Auth before database
 *
 * `getPool()`/repository construction are LAZY, inside the `archive`
 * closure below — unreachable for an unauthenticated request, since
 * `handleArchiveCourse` returns immediately on `UNAUTHENTICATED`.
 */
...
export async function POST(
  ...
  try {
    ...
    const { status, body } = await handleArchiveCourse({
    ...
    console.error("POST /api/courses/:courseId/archive: unexpected route-level error", error);
