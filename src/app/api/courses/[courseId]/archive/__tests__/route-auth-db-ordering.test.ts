...
vi.mock("@/application/course/archive-course", () => ({ archiveCourse: mocks.archiveCourse }));
...
describe("POST /api/courses/:courseId/archive — real route wiring: auth before DB construction", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("unauthenticated: 401, getPool never reached", async () => {
    ...
    const response = await POST(
      new Request("http://localhost/api/courses/x/archive", { method: "POST" }),
      makeParams(COURSE_ID),
    );

    expect(response.status).toBe(401);
    expect(mocks.getPool).not.toHaveBeenCalled();
    expect(mocks.archiveCourse).not.toHaveBeenCalled();
  });
  ...
  it("authenticated: reaches real DB construction wiring with the authenticated userId", async () => {
    ...
    mocks.archiveCourse.mockResolvedValue({
      outcome: "ARCHIVED",
      course: {
        id: COURSE_ID,
        title: "Intro",
    ...
    const [command] = mocks.archiveCourse.mock.calls[0];
