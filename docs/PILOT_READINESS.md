...
Note (dependency, not a new gate): the F-02 privacy contract is decided (Run 009 D1) but implemented only in Run 009 S3. Until then the shipped Item Analysis still shows the responder count; if the real pilot starts before S3 ships, that is a conscious human decision (operating rule: refresh only after a group answering window).
